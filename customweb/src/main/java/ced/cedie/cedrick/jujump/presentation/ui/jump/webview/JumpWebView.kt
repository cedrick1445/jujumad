...
import ced.cedie.cedrick.jujump.databinding.CustomJumpWebViewBinding
import ced.cedie.cedrick.jujump.presentation.helper.PermissionHelper
import java.io.File
...
class JumpWebView(context: Context, attrs: AttributeSet): WebView(context,attrs) {
    ...
    private var activityResultMultiplePermissionCallback: ActivityResultCallback<Map<String, Boolean>> =
        ActivityResultCallback { result: Map<String, Boolean> ->
            for ((_, value) in result) {
                if (!value) return@ActivityResultCallback
            }
            if (permissionChecker.hasAllPermissionsGranted()) {
                saveImage.execute()
            } else {
    ...
    private var activityResultMultiplePermission =
        (context as AppCompatActivity).registerForActivityResult(
            ActivityResultContracts.RequestMultiplePermissions(),
            activityResultMultiplePermissionCallback
        )
    ...
    private fun dialogList() {
        ...
        builder.setItems(items) { dialog: DialogInterface, which: Int ->
            ...
            if ("Save Picture" == items[which]) {
                if (permissionChecker.isLackPermissions()) {
                    permissionChecker.requestPermissions(activityResultMultiplePermission)
                } else {
                    saveImage.execute()
                }
            }
